...
class Sigma::ErgoStateContext::Test < Test::Unit::TestCase
  def test_ergo_state_context
    ...
    pre_header = Sigma::PreHeader.with_block_header(block_header)
    ...
    # invalid # of headers
    invalid_headers_json = Array.new(8) { header_json } 
    invalid_block_headers = Sigma::BlockHeaders.from_json(invalid_headers_json)
    assert_raises do
      ctx = Sigma::ErgoStateContext.create(pre_header: pre_header, headers: invalid_block_headers)
    end
    end
  end
end
